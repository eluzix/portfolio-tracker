...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--list-accounts", action="store_true", help="List available accounts"
    )
    ...
    parser.add_argument(
        "--reset-dividends", action="store_true", help="reset dividends and re-fetch"
    )
    parser.add_argument(
        "--reset-auth", action="store_true", help="reset google authentication token"
    )
    parser.add_argument("--clear-cache", action="store_true", help="Clear the cache")
    parser.add_argument(
        "--currency", type=str, default="USD", help="Provide a currency"
    )
    parser.add_argument(
        "--dividend-rate", type=float, default=0.25, help="Provide a dividend tax rate"
    )
    parser.add_argument(
        "--liquid",
        type=str,
        default=None,
        help="Load only liquid accounts yes/no default is None",
    )
    parser.add_argument(
        "--owner",
        type=str,
        default=None,
        help="Load only accounts owned by this person",
    )
    parser.add_argument(
        "--tags", nargs="+", help="Load only accounts with the following tags"
    )
    ...
    if args.list_accounts:
        ...
        symbols_table = Table(
            show_header=True, header_style="bold pale_turquoise1", title="Accounts"
        )
    ...
    if args.accounts is not None or args.owner is not None or args.tags is not None:
        filter_by_accounts = []
        ...
        if len(filter_by_accounts) == 0:
            console.print(
                f"[bold red]No accounts found matching specified parameters[/]"
            )
            sys.exit(0)
    ...
    # Output results
    symbols_table = Table(
        show_header=True, header_style="bold pale_turquoise1", title="Symbols Summary"
    )
    ...
    info_table.add_column(
        "Modified Dietz Yield", justify="right", style="bold green_yellow"
    )
    ...
    for i, (account, data) in enumerate(all_data.items()):
        latest_transaction = max(
            data["last_transactions"].values(), key=lambda t: t["date"]
        )
        info_table.add_row(
